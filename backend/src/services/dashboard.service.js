...
exports.getDashboardAcademia = getDashboardAcademia;
exports.getAgendaAcademia = getAgendaAcademia;
...
async function getDashboardAcademia(usuarioId, academiaId) {
    await verificarPermissaoAcademia(usuarioId, academiaId);
    const hoje = new Date();
    const inicioDia = new Date(hoje.getFullYear(), hoje.getMonth(), hoje.getDate(), 0, 0, 0);
    const fimDia = new Date(hoje.getFullYear(), hoje.getMonth(), hoje.getDate(), 23, 59, 59);
    const [totalQuadras, jogosHoje, aulasHoje, bloqueiosAtivos, professores, clientesComJogos,] = await Promise.all([
        prisma_1.prisma.quadra.count({
            where: {
                academia_id: academiaId,
            },
        }),
        prisma_1.prisma.jogo.count({
            where: {
                academia_id: academiaId,
                inicio_em: {
                    gte: inicioDia,
        ...
        prisma_1.prisma.bloqueioQuadra.count({
            where: {
                quadra: {
                    academia_id: academiaId,
                },
        ...
        prisma_1.prisma.academiaUsuario.count({
            where: {
                academia_id: academiaId,
                perfil: "PROFESSOR",
                status: "ATIVO",
        ...
        prisma_1.prisma.participanteJogo.findMany({
            where: {
                jogo: {
                    academia_id: academiaId,
                },
...
async function getAgendaAcademia(usuarioId, academiaId, data) {
    await verificarPermissaoAcademia(usuarioId, academiaId);
    const inicioDia = new Date(`${data}T00:00:00`);
    const fimDia = new Date(`${data}T23:59:59`);
    const [jogos, aulas, bloqueios] = await Promise.all([
        prisma_1.prisma.jogo.findMany({
            where: {
                academia_id: academiaId,
                inicio_em: {
                    gte: inicioDia,
        ...
        prisma_1.prisma.bloqueioQuadra.findMany({
            where: {
                quadra: {
                    academia_id: academiaId,
                },
