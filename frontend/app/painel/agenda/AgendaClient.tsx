  ...
  useEffect(() => {
    async function loadAgenda() {
      ...
      try {
        ...
        if (selectedAcademiaId && canManageAcademia(perfilNaAcademia)) {
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-zinc-500">
              Jogadores acompanham partidas e convites. Professores criam aulas avulsas e
              recorrentes sem depender de academia ativa.
            </p>
          </div>
            ...
            {convitesPendentes.length > 0 ? (
              convitesPendentes.map((convite) => (
                <article
                  ...
                  className="rounded-[24px] border border-zinc-200 bg-zinc-50 p-4"
                >
                  <p className="text-lg font-black text-zinc-950">{convite.enviadoPor.nome}</p>
                  <p className="mt-1 text-sm text-zinc-500">
          ...
          >
            <form onSubmit={handleCriarAula} className="grid gap-3 md:grid-cols-2">
              ...
              <FormField
                ...
                type="time"
                value={aulaForm.fim}
                onChange={(value) => setAulaForm((current) => ({ ...current, fim: value }))}
              />
              <FormField
                label="Observacoes"
              ...
              {recorrencias.length > 0 ? (
                recorrencias.map((recorrencia) => (
                  ...
                  >
                    ...
                    <p className="mt-1 text-sm text-zinc-500">
                      {recorrencia.horario_inicio} - {recorrencia.horario_fim} ·{" "}
                      {recorrencia.status}
                    </p>
                    <button
                      type="button"
            ...
            {agendaAcademia.length > 0 ? (
              agendaAcademia.map((evento) => (
                ...
                >
                  ...
                  <p className="mt-1 text-sm text-zinc-500">
                    {formatDateTime(evento.inicio_em)} ·{" "}
                    {formatPeriodo(evento.inicio_em, evento.fim_em)}
                  </p>
                </article>
              ))
            ) : (
